...
function App(){
 ...
 const nav=[['dashboard','Dashboard',LayoutDashboard],['project','Project',FolderKanban],['actors','Actors',Users],['usecases','Use Cases',Workflow,[['usecases','Daftar Use Case'],['modulepreview','Use Case per Modul'],['modules','Rekap per Modul']]],['factors','Factors',SlidersHorizontal],['calculation','Calculation',Calculator,[['calculation','Kalkulasi UCP'],['customcalc','Kalkulasi Custom']]],['planning','Planning',CalendarDays],['staffing','Staffing',UsersRound],['cost','Cost',WalletCards],['feasibility','Feasibility',ShieldCheck],['reports','Laporan',FileDown]];
 ...
 return <div className="app">
  ...
  <main>
   ...
   {tab==='customcalc'&&<CustomCalc s={s} c={calc} update={update} go={setTab}/>}
...
// Halaman Kalkulasi Custom. Angka masukannya, PM dan durasi, berasal dari
// Kalkulasi UCP, jadi ringkasannya ditampilkan lebih dulu agar terlihat dari
// mana kedua angka itu datang tanpa berpindah submenu.
function CustomCalc({s,c,update,go}){
 return <>
  <div className="cards">
   <Card label="Person-Month" value={c.pm.toFixed(2)} sub="dari Kalkulasi UCP"/>
   <Card label="Durasi (M)" value={`${c.duration.toFixed(2)} mo`} sub="3 × PM^(1/3)"/>
   <Card label="Mandays" value={fmt(c.mandays)} sub="person-day"/>
   <Card label="Man" value={fmt(c.man)} sub="orang"/>
  </div>
  <CustomScenario s={s} c={c} update={update} go={go}/>
 </>;
}
...
Seluruh use case itu ikut terhapus dan tidak dapat dikembalikan. UUCW proyek berkurang ${bobot}, dari ${c.uucw} menjadi ${c.uucw-bobot}.`
 ...
 // Nama modul bisa panjang, jadi diletakkan sebagai label terpisah yang dapat
 // dipotong, bukan disambung ke judul panel yang akan mendorong tombol.
 const konteks=aktif?[aktif.code,aktif.name].filter(Boolean).join(' · '):'Tanpa modul';
 return <>
  ...
  {pratinjau&&<section className="panel pratinjau">
   <div className="panel-head"><h3><span className="judul">Pratinjau Impor</span></h3><span className="total">{pratinjau.namaBerkas} · lembar {pratinjau.sheetName}</span></div>
   <div className="cards">
   ...
   <div className="tablewrap lebar"><table><caption className="sr-only">Rencana perubahan dari berkas yang diimpor</caption>
  ...
  <section className="panel">
   <div className="panel-head">
    <h3><span className="judul">Use Cases</span>{!!s.modules.length&&<span className="tag">{konteks}</span>}</h3>
    <span className="total">{tampil.length} baris</span>
   </div>
   <div className="toolbar">
    ...
    <div className="toolbar-group">
     <span className="toolbar-label">Batch Excel</span>
   ...
   <div className="tablewrap lebar"><table><caption className="sr-only">Daftar use case, jumlah transaksi, dan kompleksitasnya</caption>
